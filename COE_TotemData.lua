...
function COE:ScanTotems()
	...
	while true do
		...
		-- is this a totem?
		-- -----------------
		if (SpellName ~= COESTR_TOTEMICRECALL and SpellRank and string.find( SpellName, COESTR_SCANTOTEMS ) ~= nil) then
			local newtotem = true;
			local totem = nil;
	...
	if (minutetext) then
		...
		if (minutetext) then
			...
			if (b and sectext1) then
				...
				if (sectext2) then
					duration = math.max( tonumber( sectext1 ) or 0, tonumber( sectext2 ) );
				else
					duration = tonumber( sectext1 ) or 0;
				end
			end
		end
	end
...
function COE:InitTotemSets()
	...
	-- ----------------------
	for set = 1, getn( COE_SavedTotemSets ) do
		COE.TotemSets[ set ] = {
			Earth = nil,
			Fire = nil,
			Water = nil,
	...
	-- for each custom set
	-- --------------------
	for set = 1, getn( COE_CustomTotemSets ) do
	...
	COE.TotemSetCount = getn( COE.TotemSets );
end
