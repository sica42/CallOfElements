...
function COE_Totem:GetSetIndex( name )
	...
	-- -------------------
	local set;
	for i = 1, getn( COE_SavedTotemSets ) do
	...
	for _, regex in COESTR_TOTEMDAMAGE do
		...
		if (spellName and dmg) then
			...
			for i = 1, COE.TotemCount do
				if (COE.TotemData[ i ].SpellName == spellName) then
					-- subtract damage
					-- ----------------
					COE.TotemData[ i ].CurHealth = COE.TotemData[ i ].CurHealth - tonumber( dmg );

					-- totem destroyed?
					-- -----------------
					if (COE_Config:GetSaved( COEOPT_ENABLETIMERS ) == 1) then
						if (COE.TotemData[ i ].CurHealth <= 0) then
							COE:DebugMessage( spellName .. " destroyed" );
							...
							COE:DebugMessage( spellName .. " took " .. dmg .. " damage. " ..
